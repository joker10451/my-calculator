 ...
 * Создает папки с index.html (правильная структура для GitHub Pages)
 */
...
function generateStaticFiles() {
  ...
  console.log('🚀 Генерация статических HTML файлов (папки с index.html)...\n');
  ...
  routes.forEach(route => {
    ...
    // Создаем папку с index.html внутри
    let dirPath;
    if (route.folder === '.') {
      // Главная страница - корень dist
      dirPath = distPath;
    } else {
  ...
  // Удаляем старые .html файлы если есть
  routes.forEach(route => {
    if (route.folder !== '.') {
      const htmlFile = path.join(distPath, `${route.folder}.html`);
      if (fs.existsSync(htmlFile)) {
        fs.unlinkSync(htmlFile);
        console.log(`🗑️ Удалён: ${path.relative(distPath, htmlFile)}`);
      }
    }
  });
  
  console.log(`\n✨ Сгенерировано ${generatedCount} HTML файлов`);
  console.log('📁 Структура: папка/index.html (для GitHub Pages)');
}

// Запуск
console.log('========================================');
console.log('   Генератор статических HTML файлов');
console.log('   (папки с index.html)');
console.log('========================================\n');
